...
app.all('/current_student/update/:usn', (req, res) => {
  ...
  db.query(sql, [usn], (err, result) => {
      ...
      if (req.method === 'POST') {
          ...
          // Handling GET request
          res.render('current_student_update', { currentStudentRecord }); // Pass current student record object to the template
      }
  });
});
...
app.all('/interview/update/:interview_id', (req, res) => {
  ...
  db.query(sql, [interview_id], (err, result) => {
      ...
      if (req.method === 'POST') {
          ...
          // Handling GET request
          res.render('interview_update', { interviewRecord }); // Pass interview record object to the template
      }
  });
});
...
app.all('/applyInterview/update/:usn/:interview_id', (req, res) => {
  const { usn, interview_id } = req.params;
  ...
  db.query(sql, [usn, interview_id], (err, result) => {
      ...
      const applyInterviewRecord = result[0];
      if (req.method === 'POST') {
          // Handling POST request
          ...
          // Handling GET request
          res.render('applyInterview_update', { applyInterviewRecord }); // Pass applyInterview record object to the template
      }
  });
});
